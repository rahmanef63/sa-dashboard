...
import { MenuItem, SubMenuItem, GroupLabel, NavMainData } from '@/slices/menu/nav-main/types'
import { useMenu } from '@/slices/menu/context/MenuContext'
import { MenuItemWithStringTarget, NavGroup } from '../types/userMenu.types'
import { validateMenuItem, validateGroupLabel, validateSubMenuItem } from '../utils/menuValidation'
...
export const useUserMenu = () => {
  ...
  const handleSaveLabel = useCallback((label: GroupLabel, isEdit: boolean, selectedLabelId: string | null) => {
    try {
      ...
      if (!validateGroupLabel(label, navGroups)) return;
  ...
  const handleSaveSubMenuItem = useCallback((subItem: SubMenuItem, selectedItemId: string | null, onAddSubMenuItem: (parentId: string, subItem: SubMenuItem) => void) => {
    try {
      ...
      const navGroups = navData.groups as NavGroup[];
      if (!validateSubMenuItem(subItem, selectedItemId, navGroups)) return;
  ...
  const findMenuItem = useCallback((itemId: string): MenuItem | undefined => {
    ...
    return item ? (item as MenuItem) : undefined;
  }, [navData.groups]);
